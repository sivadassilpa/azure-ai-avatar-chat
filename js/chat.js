...
let cogSvcRegion = "northeurope";
let cogSvcSubKey = "";
let azureOpenAIEndpoint = "";
let azureOpenAIApiKey = "";
let azureOpenAIDeploymentName = "gpt-4o-mini";
let systemPrompt = "You are a helpful AI assistant.";
let privateEndpointEnabled = false;
let privateEndpoint = "";
let enableOyd = false;
let talkingAvatarCharacter = "meg";
let talkingAvatarStyle = "business";
...
function getConfigValue(elementId, configKey, fallback = "") {
  const element = document.getElementById(elementId);
  const config = window.avatarChatConfig || {};
...
function applyConfigurationDefaults() {
  ...
  Object.entries(defaults).forEach(([id, fallback]) => {
    ...
    if (element && !element.value) {
      element.value = config[id] || fallback;
    }
  });
}

function loadConfigurationFromPage() {
  cogSvcRegion = getConfigValue("speechRegion", "speechRegion", "northeurope");
  cogSvcSubKey = getConfigValue("speechKey", "speechKey").trim();
  azureOpenAIEndpoint = getConfigValue(
    ...
    .trim()
    .replace(/\/$/, "");
  azureOpenAIApiKey = getConfigValue("azureOpenAIKey", "azureOpenAIKey").trim();
  azureOpenAIDeploymentName = getConfigValue(
    "azureOpenAIDeploymentName",
    "azureOpenAIDeploymentName",
    "gpt-4o-mini"
  ).trim();
  systemPrompt = getConfigValue(
    "systemPrompt",
    "systemPrompt",
    "You are a helpful AI assistant."
  );
  talkingAvatarCharacter = getConfigValue(
    "talkingAvatarCharacter",
    "talkingAvatarCharacter",
    "meg"
  ).trim();
  talkingAvatarStyle = getConfigValue(
    "talkingAvatarStyle",
    "talkingAvatarStyle",
    "business"
  ).trim();
  ttsVoice = getConfigValue(
    "ttsVoice",
    "ttsVoice",
    "en-US-AvaMultilingualNeural"
  ).trim();
  personalVoiceSpeakerProfileID = getConfigValue(
    "personalVoiceSpeakerProfileID",
    "personalVoiceSpeakerProfileID"
  ).trim();
  sttLocales = getConfigValue(
    "sttLocales",
    "sttLocales",
  ...
  if (!cogSvcSubKey || !azureOpenAIEndpoint || !azureOpenAIApiKey) {
    alert("Please fill in Azure Speech and Azure OpenAI configuration first.");
    return false;
  }
...
function connectAvatar() {
  ...
  dataSources = [];
  //Enable Our own data is needed for future.
  if (enableOyd) {
    const azureCogSearchEndpoint = document.getElementById(
      "azureCogSearchEndpoint"
    ).value;
    const azureCogSearchApiKey = document.getElementById(
...
function handleUserQuery(userQuery, userQueryHTML, imgUrlPath) {
  ...
  fetch(url, {
    ...
    .catch((error) => {
      ...
      let chatHistoryTextArea = document.getElementById("chatHistory");
      chatHistoryTextArea.innerHTML += `<br/>Assistant: ${htmlEncode(
        "Sorry, the chat request failed. Check the browser console and Azure configuration."
      )}`;
    });
}
  ...
  if (videoElement !== null && videoElement !== undefined && sessionActive) {
    ...
    setTimeout(() => {
      ...
      if (videoElement.currentTime === videoTime) {
        ...
        if (sessionActive) {
          sessionActive = false;
          const autoReconnectAvatar = document.getElementById(
            "autoReconnectAvatar"
          );
          if (autoReconnectAvatar && autoReconnectAvatar.checked) {
            console.log(
...
function checkLastSpeak() {
  ...
  if (currentTime - lastSpeakTime > 15000) {
    if (useLocalVideo && sessionActive && !isSpeaking) {
      disconnectAvatar();
      document.getElementById("localVideo").hidden = false;
      document.getElementById("remoteVideo").style.width = "0.1px";
      sessionActive = false;
...
window.microphone = () => {
  ...
  speechRecognizer.recognized = async (s, e) => {
    if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {
      ...
      if (true) {
        //!document.getElementById("continuousConversation").checked
        document.getElementById("microphone").disabled = true;
        speechRecognizer.stopContinuousRecognitionAsync(
          () => {
    ...
    document
      ...
      .addEventListener("click", function () {
        ...
        userMessage.innerHTML +=
          '<br/><img id="picInput" src="https://samples-files.com/samples/Images/jpg/1920-1080-sample.jpg" style="width:100px;height:100px"/><br/><br/>';
      });
  } else {
    document.getElementById("userMessageBox").hidden = true;
    document.getElementById("uploadImgIcon").hidden = true;
