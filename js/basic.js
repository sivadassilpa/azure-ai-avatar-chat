...
window.startSession = () => {
  ...
  let videoCropTopLeftX = document.getElementById("videoCrop").checked
    ? 600
    : 0;
  let videoCropBottomRightX = document.getElementById("videoCrop").checked
    ? 1320
    : 1920;
...
window.speak = () => {
  ...
  let spokenSsml = `<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts' xml:lang='en-US'><voice name='${ttsVoice}'><mstts:ttsembedding speakerProfileId='${personalVoiceSpeakerProfileID}'><mstts:leadingsilence-exact value='0'/>${htmlEncode(
    spokenText
    ...
    .then((result) => {
      ...
      if (result.reason === SpeechSDK.ResultReason.SynthesizingAudioCompleted) {
        ...
        console.log(
          "[" +
            new Date().toISOString() +
            "] Unable to speak text. Result ID: " +
            result.resultId
        );
...
window.stopSpeaking = () => {
  ...
  avatarSynthesizer
    .stopSpeakingAsync()
    .then(log("[" + new Date().toISOString() + "] Stop speaking request sent."))
    .catch(log);
};
